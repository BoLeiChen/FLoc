...
class mv_depth_feature_res(nn.Module):
    def __init__(self, C=64) -> None:
        ...
        params = torch.load('./pretrained_resnet/resnet50_con.pth', map_location=torch.device('cuda'))
        res50.load_state_dict(state_dict=params, strict=False)
        self.resnet = IntermediateLayerGetter(res50, return_layers={"layer4": "feat"})
        self.conv_transpose1 = nn.ConvTranspose2d(in_channels=2048, out_channels=512, kernel_size=2, stride=2, padding=0)
        ...
        # res50 = build_model(cfg).backbone.bottom_up
        # params = torch.load('/home/cbl/f3loc/resnet50_con.pth', map_location=torch.device('cuda'))
        # res50.load_state_dict(state_dict=params, strict=False)

        # self.resnet = IntermediateLayerGetter(res50, return_layers={"res2": "feat"})
        self.C = C
    ...
    def forward(self, x, mask=None):
        ...
        Output:
            ...
            attn: (N, fW, fH)
        """
        x = self.resnet(x)["feat"]  # (N, 256, fH, fW) (8, 256, 120, 160)
        x = self.conv_transpose2(self.relu1(self.bn1(self.conv_transpose1(x))))

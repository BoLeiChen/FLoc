...
class comp_d_net(nn.Module):
    def __init__(
        ...
        mono_params = torch.load('./d_g/mono.ckpt', map_location=torch.device('cuda'))['state_dict']
        new_mono_params = {}
        for key in mono_params.keys():
            if key.startswith("encoder."):
                new_key = key.replace("encoder.", "")
                new_mono_params[new_key] = mono_params[key]
        self.mono_net.load_state_dict(state_dict=new_mono_params, strict=True)
        mv_params = torch.load('./mvd_g/mv.ckpt', map_location=torch.device('cuda'))['state_dict']
        new_mv_params = {}
        for key in mv_params.keys():
            if key.startswith("net."):
                # new_key = key.replace("net.", "")
                new_key = key[4:]
                new_mv_params[new_key] = mv_params[key]
        self.mv_net.load_state_dict(state_dict=new_mv_params, strict=True)

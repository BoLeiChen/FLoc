...
class S3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # gravity align
        img = gravity_align(img, r=self.gt_poses[idx][-2], p=self.gt_poses[idx][-1], K=K)

        # compute the attention mask
        mask = np.ones(list(img.shape[:2]))
        mask = gravity_align(mask, r=self.gt_poses[idx][-2], p=self.gt_poses[idx][-1], K=K)
        mask[mask < 1] = 0
        mask = mask.astype(np.uint8)
        img = np.transpose(img, [2, 0, 1]).astype(np.float32) # (C, H, W)
       

        data_dict = {"ref_depth": self.gt_depths[idx], "ref_img": img, "ref_mask":mask}
        if self.return_original:
